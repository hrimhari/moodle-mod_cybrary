@mod @mod_cybrary
Feature: Teachers can edit or delete any cybrary post
  In order to refine the cybrary contents
  As a teacher
  I need to edit or delete any user's cybrary posts
  ...
  Background:
    ...
    And I follow "Course 1"
    And I reply "Teacher post subject" post from "Test cybrary name" cybrary with:
  ...
  Scenario: A teacher can delete another user's posts
    ...
    And I click on "Delete" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' cybrarypost ')][contains(., 'Student post subject')]" "xpath_element"
  ...
  Scenario: A teacher can edit another user's posts
    ...
    And I follow "Test cybrary name"
    And I follow "Teacher post subject"
    And I click on "Edit" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' cybrarypost ')][contains(., 'Student post subject')]" "xpath_element"
  ...
  Scenario: A student can't edit or delete another user's posts
    ...
    Then I should not see "Edit" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' cybrarypost ')][contains(., 'Teacher post subject')]" "xpath_element"
    And I should not see "Delete" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' cybrarypost ')][contains(., 'Teacher post subject')]" "xpath_element"
