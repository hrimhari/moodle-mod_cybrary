 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_cybrary_events_testcase extends advanced_testcase {
    ...
    public function test_course_searched_searchterm_validation() {
        ...
        \mod_cybrary\event\course_searched::create($params);
    }
    ...
    public function test_course_searched() {
        ...
        $this->assertInstanceOf('\mod_cybrary\event\course_searched', $event);
        $this->assertEquals($coursectx, $event->get_context());
        $expected = array($course->id, 'cybrary', 'search', "search.php?id={$course->id}&amp;search={$searchterm}", $searchterm);
    ...
    /**
     * Ensure discussion_created event validates that cybraryid is set.
     */
    public function test_discussion_created_cybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $context = context_module::instance($cybrary->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\discussion_created::create($params);
    }
    ...
    public function test_discussion_created() {
        ...
        $discussion = $this->getDataGenerator()->get_plugin_generator('mod_cybrary')->create_discussion($record);
        ...
        $this->assertInstanceOf('\mod_cybrary\event\discussion_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'add discussion', "discuss.php?d={$discussion->id}", $discussion->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_updated event validates that cybraryid is set.
     */
    public function test_discussion_updated_cybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $context = context_module::instance($cybrary->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\discussion_updated::create($params);
    }
    ...
    public function test_discussion_updated() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\discussion_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_deleted event validates that cybraryid is set.
     */
    public function test_discussion_deleted_cybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $context = context_module::instance($cybrary->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\discussion_deleted::create($params);
    }
    ...
    public function test_discussion_deleted() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\discussion_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'delete discussion', "view.php?id={$cybrary->cmid}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_moved event validates that fromcybraryid is set.
     */
    public function test_discussion_moved_fromcybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $tocybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));

        $context = context_module::instance($tocybrary->cmid);

        $params = array(
            'context' => $context,
            'other' => array('tocybraryid' => $tocybrary->id)
        );

        $this->setExpectedException('coding_exception', 'The \'fromcybraryid\' value must be set in other.');
        \mod_cybrary\event\discussion_moved::create($params);
    }

    /**
     * Ensure discussion_moved event validates that tocybraryid is set.
     */
    public function test_discussion_moved_tocybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $fromcybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $tocybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $context = context_module::instance($tocybrary->cmid);

        $params = array(
            'context' => $context,
            'other' => array('fromcybraryid' => $fromcybrary->id)
        );

        $this->setExpectedException('coding_exception', 'The \'tocybraryid\' value must be set in other.');
        \mod_cybrary\event\discussion_moved::create($params);
    }
    ...
    public function test_discussion_moved_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $discussion->id,
            'other' => array('fromcybraryid' => $fromcybrary->id, 'tocybraryid' => $tocybrary->id)
        );
    ...
    public function test_discussion_moved() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\discussion_moved', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'move discussion', "discuss.php?d={$discussion->id}",
            $discussion->id, $tocybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_discussion_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_cybrary\event\discussion_viewed::create($params);
    }
    ...
    public function test_course_module_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_cybrary\event\course_module_viewed::create($params);
    }
    ...
    public function test_course_module_viewed() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'view cybrary', "view.php?f={$cybrary->id}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
    ...
    /**
     * Ensure subscription_created event validates that the cybraryid is set.
     */
    public function test_subscription_created_cybraryid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\subscription_created::create($params);
    }
    ...
    public function test_subscription_created() {
        ...
        $record['cybrary'] = $cybrary->id;
        $record['userid'] = $user->id;
        $subscription = $this->getDataGenerator()->get_plugin_generator('mod_cybrary')->create_subscription($record);
        ...
        $event = \mod_cybrary\event\subscription_created::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\subscription_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'subscribe', "view.php?f={$cybrary->id}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/subscribers.php', array('id' => $cybrary->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure subscription_deleted event validates that the cybraryid is set.
     */
    public function test_subscription_deleted_cybraryid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\subscription_deleted::create($params);
    }
    ...
    public function test_subscription_deleted() {
        ...
        $event = \mod_cybrary\event\subscription_deleted::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\subscription_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'unsubscribe', "view.php?f={$cybrary->id}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/subscribers.php', array('id' => $cybrary->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure readtracking_enabled event validates that the cybraryid is set.
     */
    public function test_readtracking_enabled_cybraryid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\readtracking_enabled::create($params);
    }
    ...
    public function test_readtracking_enabled() {
        ...
        $event = \mod_cybrary\event\readtracking_enabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\readtracking_enabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'start tracking', "view.php?f={$cybrary->id}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/view.php', array('f' => $cybrary->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     *  Ensure readtracking_disabled event validates that the cybraryid is set.
     */
    public function test_readtracking_disabled_cybraryid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\readtracking_disabled::create($params);
    }
    ...
    public function test_readtracking_disabled() {
        ...
        $event = \mod_cybrary\event\readtracking_disabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\readtracking_disabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'stop tracking', "view.php?f={$cybrary->id}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/view.php', array('f' => $cybrary->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     *  Ensure subscribers_viewed event validates that the cybraryid is set.
     */
    public function test_subscribers_viewed_cybraryid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'cybraryid\' value must be set in other.');
        \mod_cybrary\event\subscribers_viewed::create($params);
    }
    ...
    public function test_subscribers_viewed() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\subscribers_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'view subscribers', "subscribers.php?id={$cybrary->id}", $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_user_report_viewed_reportmode_validation() {
        ...
        \mod_cybrary\event\user_report_viewed::create($params);
    }
    ...
    public function test_user_report_viewed() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\user_report_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'user report',
    ...
    public function test_post_created_postid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($cybrary->cmid),
            'other' => array('cybraryid' => $cybrary->id, 'cybrarytype' => $cybrary->type, 'discussionid' => $discussion->id)
        );

        \mod_cybrary\event\post_created::create($params);
    }
    ...
    public function test_post_created_discussionid_validation() {
        ...
        $record['userid'] = $user->id;
        $post = $this->getDataGenerator()->get_plugin_generator('mod_cybrary')->create_post($record);

        $params = array(
            'context' => context_module::instance($cybrary->cmid),
            'objectid' => $post->id,
            'other' => array('cybraryid' => $cybrary->id, 'cybrarytype' => $cybrary->type)
        );
    ...
    /**
     *  Ensure post_created event validates that the cybraryid is set.
     */
    public function test_post_created_cybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
        ...
        $params = array(
            'context' => context_module::instance($cybrary->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'cybrarytype' => $cybrary->type)
        );
    ...
    /**
     *  Ensure post_created event validates that the cybrarytype is set.
     */
    public function test_post_created_cybrarytype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
        ...
        $params = array(
            'context' => context_module::instance($cybrary->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'cybraryid' => $cybrary->id)
        );

        $this->setExpectedException('coding_exception', 'The \'cybrarytype\' value must be set in other.');
        \mod_cybrary\event\post_created::create($params);
    }
    ...
    public function test_post_created_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'cybraryid' => $cybrary->id, 'cybrarytype' => $cybrary->type)
        );
    ...
    public function test_post_created() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'add post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/discuss.php', array('d' => $discussion->id));
    ...
    /**
     * Test the post_created event for a single discussion cybrary.
     */
    public function test_post_created_single() {
        // Setup test data.
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id, 'type' => 'single'));
        $user = $this->getDataGenerator()->create_user();
        ...
        $this->assertInstanceOf('\mod_cybrary\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'add post', "view.php?f={$cybrary->id}#p{$post->id}",
            $cybrary->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/view.php', array('f' => $cybrary->id));
        $url->set_anchor('p'.$event->objectid);
    ...
    public function test_post_deleted_postid_validation() {
        ...
        \mod_cybrary\event\post_deleted::create($params);
    }
    ...
    /**
     *  Ensure post_deleted event validates that the cybraryid is set.
     */
    public function test_post_deleted_cybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    /**
     *  Ensure post_deleted event validates that the cybrarytype is set.
     */
    public function test_post_deleted_cybrarytype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    public function test_post_deleted() {
        ...
        $discussionpost = $DB->get_records('cybrary_posts');
        ...
        $lastpost = end($posts);
        $sink = $this->redirectEvents();
        cybrary_delete_post($lastpost, true, $course, $cm, $cybrary);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the events contain the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\post_deleted', $event);
        $this->assertEquals(context_module::instance($cybrary->cmid), $event->get_context());
        $expected = array($course->id, 'cybrary', 'delete post', "discuss.php?d={$discussion->id}", $lastpost->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/discuss.php', array('d' => $discussion->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
        ...
        $sink = $this->redirectEvents();
        cybrary_delete_discussion($discussion, true, $course, $cm, $cybrary);
        $events = $sink->get_events();
        ...
        foreach ($events as $event) {
            ...
            $this->assertInstanceOf('\mod_cybrary\event\post_deleted', $event);
            $this->assertEquals(context_module::instance($cybrary->cmid), $event->get_context());
            $expected = array($course->id, 'cybrary', 'delete post', "discuss.php?d={$discussion->id}", $post->id, $cybrary->cmid);
            $this->assertEventLegacyLogData($expected, $event);
            $url = new \moodle_url('/mod/cybrary/discuss.php', array('d' => $discussion->id));
            $this->assertEquals($url, $event->get_url());
            $this->assertEventContextNotUsed($event);
    ...
    /**
     * Test post_deleted event for a single discussion cybrary.
     */
    public function test_post_deleted_single() {
        ...
        $this->assertInstanceOf('\mod_cybrary\event\post_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'delete post', "view.php?f={$cybrary->id}", $post->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/view.php', array('f' => $cybrary->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_post_updated_discussionid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'discussionid\' value must be set in other.');
        \mod_cybrary\event\post_updated::create($params);
    }

    /**
     *  Ensure post_updated event validates that the cybraryid is set.
     */
    public function test_post_updated_cybraryid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    /**
     *  Ensure post_updated event validates that the cybrarytype is set.
     */
    public function test_post_updated_cybrarytype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $cybrary = $this->getDataGenerator()->create_module('cybrary', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    public function test_post_updated() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'update post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $post->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/discuss.php', array('d' => $discussion->id));
        $url->set_anchor('p'.$event->objectid);
        $this->assertEquals($url, $event->get_url());
    ...
    public function test_post_updated_single() {
        ...
        $this->assertInstanceOf('\mod_cybrary\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'cybrary', 'update post', "view.php?f={$cybrary->id}#p{$post->id}",
            $post->id, $cybrary->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/cybrary/view.php', array('f' => $cybrary->id));
    ...
    public function test_discussion_subscription_created() {
        ...
        // Trigger the event by subscribing the user to the cybrary discussion.
        \mod_cybrary\subscriptions::subscribe_user_to_discussion($user->id, $discussion);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\discussion_subscription_created', $event);


        $cm = get_coursemodule_from_instance('cybrary', $discussion->cybrary);
    ...
    public function test_discussion_subscription_created_validation() {
        ...
        // The user is not subscribed to the cybrary. Insert a new discussion subscription.
        ...
        $event = \mod_cybrary\event\discussion_subscription_created::create($params);
    ...
    public function test_discussion_subscription_created_validation_contextlevel() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_cybrary\event\discussion_subscription_created::create($params);
    }
    ...
    /**
     * Test cybraryid validation of discussion_subscription_created event.
     */
    public function test_discussion_subscription_created_validation_cybraryid() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/mod/cybrary/lib.php');
    ...
    public function test_discussion_subscription_deleted() {
        ...
        // Trigger the event by unsubscribing the user to the cybrary discussion.
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_cybrary\event\discussion_subscription_deleted', $event);
    ...
    public function test_discussion_subscription_deleted_validation() {
        ...
        $subscription->cybrary = $cybrary->id;
        $subscription->discussion = $discussion->id;
        $subscription->preference = \mod_cybrary\subscriptions::CYBRARY_DISCUSSION_UNSUBSCRIBED;
        ...
        $event = \mod_cybrary\event\discussion_subscription_deleted::create($params);
    ...
    public function test_discussion_subscription_deleted_validation_contextlevel() {
        ...
        // Without an invalid context.
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_cybrary\event\discussion_subscription_deleted::create($params);
    }
    ...
    /**
     * Test cybraryid validation of discussion_subscription_deleted event.
     */
    public function test_discussion_subscription_deleted_validation_cybraryid() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/mod/cybrary/lib.php');
     ...
     * users.
     */
    public function test_cybrary_subscription_page_context_valid() {
        global $CFG, $PAGE;
        require_once($CFG->dirroot . '/mod/cybrary/lib.php');
        ...
        // Set up the default page event to use this cybrary.
        ...
        // Trigger the event by subscribing the user to the cybrary.
        ...
        // Trigger the event by unsubscribing the user to the cybrary.
    ...
    /**
     * Test mod_cybrary_observer methods.
     */
    public function test_observers() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $trackedrecord = array('course' => $course->id, 'type' => 'general', 'forcesubscribe' => CYBRARY_INITIALSUBSCRIBE);
        $untrackedrecord = array('course' => $course->id, 'type' => 'general');
        $trackedcybrary = $this->getDataGenerator()->create_module('cybrary', $trackedrecord);
        $untrackedcybrary = $this->getDataGenerator()->create_module('cybrary', $untrackedrecord);
        ...
        // The role_assign observer does it's job adding the cybrary_subscriptions record.
        ...
        set_config('cybrary_trackingtype', 1);
        set_config('cybrary_trackreadposts', 1);

        $record = array();
        $record['course'] = $course->id;
        $record['cybrary'] = $trackedcybrary->id;
        $record['userid'] = $user->id;
        ...
        $record['discussion'] = $discussion->id;
        $record['userid'] = $user->id;
        $post = $cybrarygen->create_post($record);

        cybrary_tp_add_read_record($user->id, $post->id);
        cybrary_set_user_maildigest($trackedcybrary, 2, $user);
        cybrary_tp_stop_tracking($untrackedcybrary->id, $user->id);

        $this->assertEquals(1, $DB->count_records('cybrary_subscriptions'));
        $this->assertEquals(1, $DB->count_records('cybrary_digests'));
        $this->assertEquals(1, $DB->count_records('cybrary_track_prefs'));
        $this->assertEquals(1, $DB->count_records('cybrary_read'));

        // The course_module_created observer does it's job adding a subscription.
        $cybraryrecord = array('course' => $course->id, 'type' => 'general', 'forcesubscribe' => CYBRARY_INITIALSUBSCRIBE);
        $extracybrary = $this->getDataGenerator()->create_module('cybrary', $cybraryrecord);
        $this->assertEquals(2, $DB->count_records('cybrary_subscriptions'));

        $manplugin->unenrol_user($manualenrol, $user->id);

        $this->assertEquals(0, $DB->count_records('cybrary_digests'));
        $this->assertEquals(0, $DB->count_records('cybrary_subscriptions'));
        $this->assertEquals(0, $DB->count_records('cybrary_track_prefs'));
        $this->assertEquals(0, $DB->count_records('cybrary_read'));
    }
